...
export function ScanProductDetail() {
    ...
    if (!product) return (
        <div className="text-center py-20 font-bold text-gray-400 uppercase px-6">
            Asset not found in registry.
        </div>
    );

    // Safely derive booleans for every optional field
    const hasProductNumber  = !!product.productNumber;
    const hasName           = !!product.name;
    const hasDescription    = !!product.description;
    const hasGemSize        = !!product.gem_size;
    const hasOrigin         = !!product.more_information?.origin;
    const hasDetails        = product.details && Object.keys(product.details).length > 0;
    const hasWeight         = !!product.more_information?.weight;
    const hasLocation       = !!product.location;
    ...
    const availableTabs = [
        { id: 'description', label: 'Description', show: true },
        {
            id: 'certificate',
            label: 'Certificate',
            show: hasLabTestImg || hasCertificateImg,
        },
    ].filter(tab => tab.show);

    // Ensure activeTab is always valid (e.g. if cert tab disappears)
    const resolvedActiveTab = availableTabs.find(t => t.id === activeTab)
        ? activeTab
        : availableTabs[0]?.id;

    // Description tab has meaningful content to display
    const hasDescriptionContent = hasGemSize || hasOrigin || hasDetails || hasWeight || hasLocation;
    ...
    return (
        <section className="flex flex-col w-full lg:max-w-[650px] px-4 sm:px-6 mx-auto mb-12 animate-in fade-in duration-700 overflow-x-hidden">
            ...
            <div className="flex items-center gap-2 mb-4 md:mb-6 bg-emerald-50 w-fit px-3 md:px-4 py-1.5 rounded-full border border-emerald-100">
                ...
                <span className="text-[9px] md:text-[10px] font-black uppercase tracking-wider text-emerald-700">
            ...
            {/* Navigation Tabs — only render if more than one tab exists */}
            ...
            <div className="min-h-[250px] mb-8 md:mb-10">
                {resolvedActiveTab === 'description' && (
                    <div className="animate-in slide-in-from-bottom-2 duration-500 space-y-6 md:space-y-8">
                        ...
                        {/* Essential Specs Grid — only if at least one stat is available */}
                        {(hasGemSize || hasOrigin) && (
                            <div className="grid grid-cols-2 gap-3 md:gap-4">
                                {hasGemSize && (
                                    <div className="bg-white border border-gray-100 p-3 md:p-4 rounded-xl md:rounded-2xl shadow-sm">
                                        <p className="text-[8px] md:text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">
                                            Dimensions
                                        </p>
                        ...
                        {/* Composition Details — only if at least one non-empty detail or weight exists */}
                        {(() => {
                            const validDetails = hasDetails
                                ? Object.entries(product.details).filter(
                                    ([, val]) => val !== null && val !== undefined && val !== ''
                                  )
                                : [];
                            const showSection = validDetails.length > 0 || hasWeight;
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2.5 md:gap-y-3 gap-x-8 border-t border-gray-100 pt-4">
                                        {validDetails.map(([key, val]) => (
                                            <div key={key} className="flex justify-between items-center py-0.5">
                                                <span className="text-[10px] md:text-xs text-gray-400 uppercase font-bold">
                                                    {key.replace('_', ' ')}
                                                </span>
                        ...
                        {hasLocation && (
                            <div className="flex items-center gap-3 text-gray-500 bg-gray-50 p-3.5 md:p-4 rounded-xl md:rounded-2xl border border-gray-100">
                                ...
                                <span className="text-[9px] md:text-xs font-bold uppercase tracking-wider truncate">
                                    Storage: {product.location}
                                </span>
                            </div>
                        )}

                        {/* Fallback when no description content is available */}
            ...
            {/* Market Value Footer — only render if price exists */}
            {hasPrice && (
                <div className="w-full bg-gray-900 p-6 md:p-8 rounded-[1.8rem] md:rounded-[2.5rem] shadow-2xl text-center relative overflow-hidden">
                    <div className="relative z-10">
                        <p className="text-pink-400/60 text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] mb-1 md:mb-2">
                            Current Valuation
                        </p>
                        <p className="font-black text-2xl xs:text-3xl md:text-4xl text-white tracking-tighter leading-none">
                            ...
                            <span className="text-sm md:text-lg text-pink-500 ml-1">PKR</span>
                        </p>
                    </div>
                    <div className="absolute top-0 right-0 w-24 md:w-32 h-24 md:h-32 bg-[#CA0A7F]/20 blur-[40px] md:blur-[60px] rounded-full" />
                    <div className="absolute bottom-0 left-0 w-24 md:w-32 h-24 md:h-32 bg-blue-500/10 blur-[40px] md:blur-[60px] rounded-full" />
                </div>
            )}
        </section>
    );
