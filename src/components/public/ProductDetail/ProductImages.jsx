...
export function ProductImages({ product }) {
  ...
  // Sync main image if the product ID changes or when images finally load
  useEffect(() => {
    if (images.length > 0) {
      setMainImage(images[0]);
    }
  }, [product?._id, images.length]); // Added images.length to dependency to catch late loads
  ...
  // 2. Return null or a skeleton if no product exists yet to prevent rendering empty UI
    ...
    <section className="flex flex-col gap-5 w-full lg:w-[600px]">
      ...
      >
        {mainImage ? (
          <img
            src={mainImage}
            alt={`Authentic ${product.name || 'Product'} Display`}
