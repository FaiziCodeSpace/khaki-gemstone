...
export function Card({ products }) {
    ...
    if (!products || products.length === 0) {
        return (
            <div className="flex-5 text-center py-20 text-gray-400">
                No gemstones match your selection.
            </div>
        );
    }
        ...
        <section className="flex flex-col mid:flex-row mid:flex-wrap gap-10 items-start justify-center px-6 mid:px-12 pb-12 flex-5 w-full">
            {products.map((item) => (
                <Link
                    key={item._id}
                    to={`/product/${item._id}`}
                    className="group relative flex flex-row mid:flex-col bg-white p-3 mid:p-4 rounded-[24px] mid:rounded-[32px] w-full mid:w-[300px] shadow-sm hover:shadow-xl mid:hover:-translate-y-2 transition-all duration-500 ease-out cursor-pointer overflow-hidden block"
                >
                    {/* Like Button */}
                    <button
                    ...
                    <div className="flex flex-col justify-between flex-1 ml-4 mid:ml-0 mid:mt-4 mid:px-1 min-w-0">
                        ...
                        <div className="mt-auto pt-2 mid:mt-5 flex justify-between items-center gap-2">
                            ...
                            <button
                                ...
                                className="active:scale-95 bg-[#2D2636] hover:bg-black px-3 mid:px-6 py-2 mid:py-2.5 text-[10px] mid:text-sm font-medium text-white rounded-full shadow-lg transition-all duration-300 cursor-pointer flex-shrink-0"
                            >
                                Buy Now
                            </button>
                        </div>
