export function PriceDetails({ cartItems }) {
    ...
    const subtotal = cartItems.reduce((total, item) => total + getItemPrice(item), 0);
                ...
                <ul className="flex flex-col gap-4">
                    {cartItems.map((item) => (
                        <li key={item._id} className="flex justify-between font-satoshi text-[16px]">
                            <p className="font-normal text-[#737272] line-clamp-1 pr-4">{item.name}</p>
                    ...
                    <li className="flex justify-between font-satoshi">
                        ...
                        <p className="font-medium text-[#282930]">{delivery.toLocaleString()} PKR</p>
                    </li>
